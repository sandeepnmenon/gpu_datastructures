...
int main(void)
{
    ...
    auto pairs_begin = thrust::make_transform_iterator(
        ...
        [] __device__(auto i)
        { return cuco::make_pair(custom_key_type{i}, custom_value_type{i}); });
    ...
    assert(
        thrust::all_of(contained.begin(), contained.end(), [] __device__(auto const &b)
                       { return b; }));
